...
int read_problem_dense_2(const mxArray *label_vec, const mxArray *instance_mat, const mxArray *upper_bound, const mxArray *linear_term, const mxArray * initial_point)
{
	int i, j, k;
	int elements, max_index, sc;
	double *samples, *labels, *upperbound, *linearterm, *alpha;

	labels = mxGetPr(label_vec);
	samples = mxGetPr(instance_mat);
	sc = mxGetN(instance_mat);
	
	upperbound = mxGetPr(upper_bound);
	linearterm = mxGetPr(linear_term);
	alpha = mxGetPr(initial_point);
	...
	prob.upperbound = Malloc(double,prob.l);
	prob.linearterm = Malloc(double,prob.l);
	...
	for(i=0; i<prob.l;i++)
	{
		prob.upperbound[i] = upperbound[i];
		prob.linearterm[i] = linearterm[i];
...
// prhs[2]: options prhs[3]: upper bound of variables
// prhs[4]: linear term [5]: initial point
		...
		int nrhs, const mxArray *prhs[] )
{
	...
	if(nrhs > 0 && nrhs < 7)
	{
		...
		else
		{
            ...
            if(nrhs == 6)
      	        err = read_problem_dense_2(prhs[0], prhs[1],prhs[3],prhs[4],prhs[5]);
    }
		...
		if(err || error_msg)
		{
			...
			free(prob.x);
			free(x_space);
			free(prob.upperbound);
			free(prob.linearterm);
			free(prob.alpha);
			fake_answer(plhs);
			return;
		}
