...
public:
	...
	void Solve2(int l, const QMatrix& Q, const double *b_, const schar *y_,
		   double *alpha_, double* upperbound, double eps,
		   SolutionInfo* si, int shrinking);	   
protected:
	int active_size;
	schar *y;
...
void Solver::swap_index(int i, int j)
{
	...
	swap(upperbound[i],upperbound[j]);
}
...
void Solver::Solve2(int l, const QMatrix& Q, const double *b_, const schar *y_,
		   double *alpha_, double* upperbound_, double eps,
		   SolutionInfo* si, int shrinking)
{
	this->l = l;
	this->Q = &Q;
	...
	clone(y, y_,l);
	clone(alpha,alpha_,l);
	clone(upperbound,upperbound_,l);
	//this->Cp = Cp;
	//this->Cn = Cn;
	this->eps = eps;
	unshrinked = false;
	...
	/*
	this->upperbound = Malloc(double,l);
	for (int i =0; i < l; i++)
	...
	double *alpha, Solver::SolutionInfo* si, double Cp, double Cn)
{
	...
	double *upperbound = new double[l];
	schar *y = new schar[l];
	...
	for(i=0;i<l;i++)
	{
		...
		//minus_ones[i] = -1;
		minus_ones[i] = prob->linearterm[i];
		upperbound[i] = prob->upperbound[i];
...
//		alpha, Cp,Cn, param->eps, si, param->shrinking);
 	...
 	s.Solve2(l, SVC_Q(*prob,*param,y), minus_ones, y,
		alpha, upperbound, param->eps, si, param->shrinking);
	...
	double Cp, double Cn)
{
	...
	for(int i=0;i<prob->l;i++)
	{
		if(fabs(alpha[i]) > 0)
		{
			...
			if(prob->y[i] > 0)
			{
				if(fabs(alpha[i]) >= prob->upperbound[i])
					++nBSV;
			}
			else
			{
  ...
  /*
    ...
    */
    ...
    int l = prob->l;
		...
		svm_group_classes(prob,&nr_class,&label,&start,&count,perm);	
    decision_function *f = Malloc(decision_function,nr_class*(nr_class-1)/2);
    f[0] = svm_train_one(prob,param,1,1);
    ...
    model->nSV = NULL;
		...
		for(int i=0;i<nr_class-1;i++)
			model->sv_coef[i] = Malloc(double,prob->l);

    for(int i=0; i < prob->l; i++)
     model->sv_coef[0][i] = f[0].alpha[i];
